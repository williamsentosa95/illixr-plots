...
fig, ax = plt.subplots()
data_path = "data/compression_ate.txt"
...
for line in textfile:
    if (line_num == 1): # process the header
        ...
        easy.append(float(entries[1]))
        medium.append(float(entries[2]))
        hard.append(float(entries[3]))
    line_num += 1
...
print(easy)
ax.plot(rtts, easy, marker='o', color='b', label=headers[1])
ax.plot(rtts, medium, marker='^', color='g', label=headers[2])
ax.plot(rtts, hard, marker='s', color='r', label=headers[3])
...
plt.savefig('compression_ape.pdf', bbox_inches='tight')
